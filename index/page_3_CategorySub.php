...
if(isset($_GET['DELCategorySub'])) {
    $ctgs_code = $_GET['DELCategorySub'];
    $Del_1= $sql_process->fastQuery("UPDATE  tbl_category_sub SET is_delete='0'   WHERE ctgs_code='$ctgs_code'");
      echo "<script>";
      echo "location.href = '?zone=CategorySub&success'";
      echo "</script>";
    }
...
<?php } ?>
                  ...
                  <div class="card">
                    <div class="card-body">
                      <h4 class="card-title"><?=$ctgs_name_main_x?>
                      <div align="right"> <button type="button" onclick="window.location.href='?zone=CategorySubForm'" class="btn btn-success mr-2">
                     <!-- เพิ่มข้อมูล -->
...
if($q != NULL ){
  $stateSQL="AND tbl_category_sub.ctgs_name LIKE '%$q%'";
}else{
  $stateSQL=NULL;
}

$total_data =$sql_process->rowsQuery("SELECT tbl_category_sub.ctgs_code FROM tbl_category_sub WHERE tbl_category_sub.is_delete ='1' AND tbl_category_sub.cmn_code='$cmn_codex' $stateSQL");
$rows='10';
if($page<=0)$page=1;
...
?>
      ...
      <td><?=$rowData->ctgm_name?></td>
        <td align="center" >
            <?php
            if($rowData->ctgs_status=='1'){
              echo '<label class="badge badge-success">';
              echo $open;
              echo "</label>";
            }elseif ($rowData->ctgs_status=='0') {
          ...
          <td>
          <?=$rowData->ctgs_description?>
        </td>
      
        <td align="center">
            <button type="button" onclick="window.location.href='?zone=CategorySubEdit&csc=<?=$rowData->ctgs_code?>'"  class="btn btn-info mr-2"><?=$f1?></button>
  <button type="button" onclick="move<?=$rowData->ctgs_code?>();" class="btn btn-danger mr-2"><?=$f2?></button>


  
  <script>
                        function move<?=$rowData->ctgs_code?>() {
                          swal({
                            title: "<?=$f2?> <?=$rowData->ctgs_name?>",
                            text: "",
                            type: "warning",
                          ...
                          function(isConfirm){
                            if (isConfirm) {
                              swal("<?=$f5?>", "<?=$f6?>", "success");
                            location.href = '?zone=<?=$zone?>&DELCategorySub=<?=$rowData->ctgs_code?>';
                            } else {
                              swal("<?=$f7?>", "<?=$f6?>", "error");
                            }
                          });
