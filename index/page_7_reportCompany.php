...
YEAR(tbl_evaluation_result.evltp_date) = '$current_year' $stageSql");
...
YEAR(tbl_evaluation_cach.evalc_date) = '$current_year' $stageSql");
...
<div class="row">
            ...
            <div class="card">
              <div class="card-body">
                <h4 class="card-title">
                  <!-- บริษัท / องค์กร -->
                  <?=$auth_user->mf("XSBMHS8Q0OJOT7VRTLNA",$country_idx);?>
...
<br>
    ...
    <?php if($eltp ==NULL){ echo "disabled";}?> class="login100-form-btn" onclick="window.location.href='?zone=ReportChk&eltp=<?=$eltp?>&evltp=<?=$evltp_code?>&eltp_status_topic=<?=$eltp_status_topic?>'">
      <!-- ดูรายละเอียด  -->
    ...
    <h3>
    <!-- ประเมินทั้งหมด -->
    <?=$auth_user->mf("VKBZXA7RI3TBF9IXRJ63",$country_idx)?>
     <?=$total_chktResult?> 
     <!-- ครั้ง -->
