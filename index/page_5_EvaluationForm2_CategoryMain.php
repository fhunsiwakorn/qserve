 ...
 <div class="col-12">
                  <div class="card">
                    <div class="card-body">
                      ...
                      <b><?=$dataRow["eltp_name"]?></b> (<?=$ctgm_name_main_x ?>)</h4>
...
$total_data =$sql_process->rowsQuery("SELECT tbl_category_main.ctgm_code FROM tbl_category_main WHERE  tbl_category_main.ctgm_status ='1' AND tbl_category_main.is_delete ='1' AND tbl_category_main.cmn_code='$cmn_codex' $stateSQL");
$rows='5';
if($page<=0)$page=1;
...
$num_gen++;
$Chk_permission =$sql_process->rowsQuery("SELECT tbl_evaluation_permission.evltp_id FROM tbl_evaluation_permission WHERE  cmn_code='$cmn_codex' AND  eltp_code ='$eltp_code_get' AND evltp_code='$rowData->ctgm_code'");
// class='table-success'
?>
      <tr <?php if($Chk_permission>=1){ echo "class='table-success'";} ?>   >
        ...
        <td><?=$rowData->ctgm_name?></td>
          ...
          <button type="button"  class="btn btn-icons btn-rounded btn-primary" onclick="window.location.href='page_5_EvaluationFormChk.php?cmn_code=<?=$cmn_codex?>&eltp=<?=$eltp_code_get?>&evltp_code_add=<?=$rowData->ctgm_code?>&page=<?=$page?>&q=<?=$q?>&#example1'" title="<?=$tx1?> <?=$rowData->ctgm_name?>" >
                          <i class="mdi mdi-key-plus"></i>
         </button>
          <?php } ?>
          <?php if($Chk_permission>=1){ ?>  
          <a  class="btn btn-icons btn-rounded btn-warning" href="page_5_EvaluationFormChk.php?cmn_code=<?=$cmn_codex?>&eltp=<?=$eltp_code_get?>&evltp_code_del=<?=$rowData->ctgm_code?>&page=<?=$page?>&q=<?=$q?>#example1"  onClick="javascript:return confirm('<?=$tx2?> <?=$rowData->ctgm_name?>');"  title="<?=$tx2?> <?=$rowData->ctgm_name?>">
