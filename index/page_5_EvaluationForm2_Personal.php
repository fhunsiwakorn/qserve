                    ...
                    <div class="card-body">
                      <h4 class="card-title">
                        ...
                        <?=$auth_user->mf("I87BGAXL6XYC3K78WWRG",$country_idx);?> 
                         <b><?=$dataRow["eltp_name"]?></b> (<?=$auth_user->mf("82BS90L39GV79H0CHXJ",$country_idx);?>)</h4>
...
$num_gen++;
$Chk_permission =$sql_process->rowsQuery("SELECT tbl_evaluation_permission.evltp_id FROM tbl_evaluation_permission WHERE  cmn_code='$cmn_codex' AND  eltp_code ='$eltp_code_get' AND evltp_code='$rowData->user_code'");
// class='table-success'
?>
      <tr <?php if($Chk_permission>=1){ echo "class='table-success'";} ?>   >
       ...
       <td><?=$rowData->ctgs_name?></td>
         ...
         <button type="button"  class="btn btn-icons btn-rounded btn-primary" onclick="window.location.href='page_5_EvaluationFormChk.php?cmn_code=<?=$cmn_codex?>&eltp=<?=$eltp_code_get?>&evltp_code_add=<?=$rowData->user_code?>&page=<?=$page?>&q=<?=$q?>&#example1'" title="<?=$tx1?> <?=$rowData->user_firstname?> <?=$rowData->user_lastname?>" >
         ...
         <a  class="btn btn-icons btn-rounded btn-warning" href="page_5_EvaluationFormChk.php?cmn_code=<?=$cmn_codex?>&eltp=<?=$eltp_code_get?>&evltp_code_del=<?=$rowData->user_code?>&page=<?=$page?>&q=<?=$q?>#example1"  onClick="javascript:return confirm('<?=$tx2?> <?=$rowData->user_firstname?> <?=$rowData->user_lastname?>');"  title="<?=$tx2?> <?=$rowData->user_firstname?> <?=$rowData->user_lastname?>">
