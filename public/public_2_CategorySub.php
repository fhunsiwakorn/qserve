...
//user Image
  ...
  $strFileImg=base64_encode_image($pathuserimg);
    ...
    if($Chk_data <=0){
   include ("public_2_CategorySubAll.php");
    }elseif(!isset($_GET["zone"])){
   include ("public_2_CategorySubEvaTopic.php");
   }else{
   switch($_GET["zone"]) {
   case "EvaTopic" : include ("public_2_CategorySubEvaTopic.php");
   break;
   case "EvaStart" : include ("public_2_CategorySubStart.php");
   break;
